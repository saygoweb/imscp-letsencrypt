...
/**
 * Activate LetsEncrypt for the given customer
 *
 * @throws DatabaseException
 * @param int $customerId Customer unique identifier
 */
function letsencrypt_activate($customerId)
{
    $stmt = exec_query(
        '
    ...
    if ($stmt->rowCount()) {
        ...
        try {
            ...
            exec_query(
                'INSERT INTO letsencrypt (admin_id, domain_id, domain_name, letsencrypt_status) VALUES (?, ?, ?, ?)',
                array($customerId, $row['domain_id'], $row['domain_name'], 'toadd')
            );

            exec_query(
                '
                    INSERT INTO letsencrypt (admin_id, domain_id, alias_id, domain_name, letsencrypt_status)
            ...
            $db->commit();
            send_request();
            set_page_message(tr('LetsEncrypt support scheduled for activation. This can take few seconds.'), 'success');
...
/**
 * Deactivate LetsEncrypt for the given customer
 *
 * @param int $customerId Customer unique identifier
 * @return void
 */
function letsencrypt_deactivate($customerId)
{
    $stmt = exec_query(
    ...
    if ($row['cnt']) {
        exec_query('UPDATE letsencrypt SET letsencrypt_status = ? WHERE admin_id = ?', array('todelete', $customerId));
        send_request();
        set_page_message(tr('LetsEncrypt support scheduled for deactivation. This can take few seconds.'), 'success');
    } else {
        showBadRequestErrorPage();
    }
}
...
/**
 * Generate customer list for which LetsEncrypt can be activated
 *
 * @param $tpl TemplateEngine
 * @return void
 */
function _letsencrypt_generateCustomerList($tpl)
{
    $stmt = exec_query(
        '
            SELECT admin_id, admin_name FROM admin WHERE created_by = ?
            AND admin_status = ? AND admin_id NOT IN (SELECT admin_id FROM letsencrypt)
...
{
    _letsencrypt_generateCustomerList($tpl);
    ...
    $stmt = exec_query(
        '
            SELECT COUNT(admin_id) AS cnt FROM admin INNER JOIN letsencrypt USING(admin_id)
    ...
    if ($rowCount) {
        $stmt = exec_query(
            "
                SELECT admin_name, admin_id FROM admin INNER JOIN letsencrypt USING(admin_id)
        ...
        while ($row = $stmt->fetchRow()) {
            $stmt2 = exec_query(
                '
                    SELECT letsencrypt_id, domain_name, letsencrypt_status, domain_dns, domain_text FROM letsencrypt
                ...
                ',
                array('LetsEncrypt_Plugin', $row['admin_id'])
            );

            if ($stmt2->rowCount()) {
                while ($row2 = $stmt2->fetchRow()) {
                    if ($row2['letsencrypt_status'] == 'ok') {
                        $statusIcon = 'ok';
                    } elseif ($row2['letsencrypt_status'] == 'disabled') {
                        $statusIcon = 'disabled';
                    } elseif (in_array($row2['letsencrypt_status'], array(
                    ...
                    $tpl->assign(array(
                        'KEY_STATUS'  => translate_dmn_status($row2['letsencrypt_status']),
                        ...
                        'LETSENCRYPT_ID' => tohtml($row2['letsencrypt_id'])
                    ));
            ...
            $tpl->assign(array(
                'TR_CUSTOMER'   => tr('LetsEncrypt entries for customer: %s', decode_idna($row['admin_name'])),
                'TR_DEACTIVATE' => tr('Deactivate LetsEncrypt'),
        ...
        set_page_message(tr('No customer with LetsEncrypt support has been found.'), 'static_info');
    }
}
...
if (isset($_REQUEST['action'])) {
    ...
    if (isset($_REQUEST['admin_id']) && $_REQUEST['admin_id'] != '') {
        ...
        switch ($action) {
            case 'activate':
                letsencrypt_activate($customerId);
                break;
            case 'deactivate';
                letsencrypt_deactivate($customerId);
                break;
            default:
                showBadRequestErrorPage();
        }
...
$tpl->define_dynamic(array(
    ...
    'page'             => '../../plugins/LetsEncrypt/themes/default/view/reseller/letsencrypt.tpl',
    'page_message'     => 'layout',
    'select_list'      => 'page',
...
$tpl->assign(array(
    ...
    'TR_ACTIVATE_ACTION'      => tr('Activate LetsEncrypt for this customer'),
    'TR_DOMAIN_NAME'          => tr('Domain Name'),
    'TR_DOMAIN_KEY'           => tr('LetsEncrypt domain key'),
    'TR_STATUS'               => tr('Status'),
    'TR_DNS_NAME'             => tr('Name'),
    'DEACTIVATE_DOMAIN_ALERT' => tojs(tr('Are you sure you want to deactivate LetsEncrypt for this customer?')),
